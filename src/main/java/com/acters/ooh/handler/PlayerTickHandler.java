...
class PlayerTickHandler implements ITickHandler
{
					...
					if (isClient())
					{
						...
						if (Minecraft.getMinecraft().thePlayer.username.equals(player.username))
						{
							...
							if ((!(idsMatch && metaMatch && sizeMatch) && bool))
							{
								...
								PacketDispatcher.sendPacketToServer(EnumPacketTypes.populatePacket(new PacketSetData(player, data)));
							}
	...
	private void attemptBreakBlock (EntityPlayer player)
	{
		...
		if (currentBlockBreak >= 1F && target != null)
		{
			...
			{
				PacketDispatcher.sendPacketToServer(EnumPacketTypes.populatePacket(new PacketBreakBlock(player, x, y, z)));
			}
