...
public class FirstPersonRenderHandler
{
	...
	private void renderHand (EntityRenderer renderer, float partialTicks)
	{
		if (renderer.debugViewDirection <= 0)
		{
			...
			try
			{
				Project.gluPerspective((Float) getFOVModifier.invoke(renderer, new Object[]{ partialTicks, false }), (float) Minecraft.getMinecraft().displayWidth / (float) Minecraft.getMinecraft().displayHeight, 0.05F, farPlaneDistance * 2.0F);
			}
			catch (Exception e)
			{
				e.printStackTrace();
			...
			try
			{
				hurtCameraEffect.invoke(renderer, new Object[]{ partialTicks });
			}
			catch (Exception e)
			{
				e.printStackTrace();
			...
			if (Minecraft.getMinecraft().gameSettings.viewBobbing)
			{
				try
				{
					setupViewBobbing.invoke(renderer, new Object[]{ partialTicks });
				}
				catch (Exception e)
				{
					e.printStackTrace();
	...
	private void renderItemInFirstPerson (ItemRenderer renderer, float partialTicks) throws Exception
	{
		...
		if (itemstack != null && itemstack.getItem() instanceof ItemMap)
		{
			...
			if (custom == null)
			{
				if (mapdata != null)
				{
					//renderer(Minecraft.getMinecraft().thePlayer, Minecraft.getMinecraft().getTextureManager(), mapdata);
				}
			}
			else
			{
